...
const invalid5 = [19, 3, 8, 2, 0, 1, 9, 7, 7, 2, 8, 8, 3, 8, 5, 4];
...
const validateCred = (arr) => {
  let tempSum = 0;
  let sum = 0;
  for (let count = arr.length-1; count >= 0; count -= 2) {
      sum += arr[count];
     } 
  for (let count = arr.length-2; count >= 0; count -= 2) {
       tempSum = arr[count] * 2;
       if (tempSum > 9) {
         sum += (tempSum - 9);
       } else {
...
const invalidCards = (nestedArray) => {
  ...
  for(let counter = 0; counter < nestedArray.length; counter++) {
    if (!validateCred(nestedArray[counter])) {
      invalids.push(nestedArray[counter]);
    }
  }
  return invalids;
}

//create function to find Card Issuer...
const idInvalidCardCompanies = (invalidNums) => {
  let companyArray = [];
  for (let counter = 0; counter < invalidNums.length; counter++) {
    
    switch (invalidNums[counter][0]) {
      ...
      case 6: 
        ...
        break;
      default:
        console.log("No known card provider for: " + invalidNums[counter].join(""));
        break;
    }
  }
  return companyArray;
